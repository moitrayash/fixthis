...
(() => {
  ...
  function landing() {
    const node = el(
      '<section class="screen landing">' +
        ...
        '<p>Tap the button. Take a photo. Tell us what’s wrong. We’ll send it to whoever can fix it.</p>' +
  ...
  function photoStep() {
    const node = el(
      '<section class="screen">' +
        '<div class="stack">' +
          ...
          '<p class="step-sub">A picture helps the right team show up with the right tools.</p>' +
          ...
          '<button class="btn btn-ghost" id="skipPhoto">Skip photo</button>' +
        '</div>' +
      '</section>'
    );
    ...
    const skipBtn = node.querySelector("#skipPhoto");
    ...
    skipBtn.addEventListener("click", function () { render(describeStep()); });
    return node;
  }
  ...
  function describeStep() {
    ...
    desc.addEventListener("input", function () {
      ...
      fixBtn.disabled = desc.value.trim().length < 3;
    });
    ...
    fixBtn.disabled = (state.description || "").trim().length < 3;
  ...
  function successScreen(report, isEdit) {
    ...
    const node = el(
      '<section class="screen success">' +
        '<div class="check">✓</div>' +
        '<h2>' + headline + '</h2>' +
        '<p style="margin:0;color:var(--muted);max-width:32ch">' + (isEdit ? "We re-sent your report to" : "We’ve routed your report to") + ' <strong>' + owner.scope + '</strong>' + ownerPhone + '.</p>' +
    ...
    node.querySelector("#anotherBtn").addEventListener("click", function () {
      ...
      state.extraFiles = []; state.geo = null;
  ...
  document.addEventListener("click", function (e) {
    if (e.target && e.target.id === "aboutLink") {
      e.preventDefault();
