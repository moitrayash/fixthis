/* Fix This — canonical data schema (v1).
   ----------------------------------------------------------------
   ...
   Every field is documented in docs/SCHEMA.md and mirrored in
   docs/openapi.yaml. Treat this file as the contract.
*/
(function () {
  "use strict";
  ...
  // DRO keys are defined in routing.js; we accept any uppercase string here.
  const DRO_PATTERN = /^[A-Z][A-Z0-9_]{1,31}$/;
  const ID_PATTERN  = /^FIX-\d{6}-[A-Z0-9]{4}$/;

  // ---------- Field spec ----------
  // Each entry: { type, required, default, enum?, validate? }
  const FIELDS = {
    ...
    photo:        { type: "string",  required: false, default: null },     // data URL or https URL
    photo_meta:   { type: "object",  required: false, default: null },     // { width, height, originalSize, compressedSize, takenAt? }
    photo_taken_at: { type: "string", required: false, default: null },    // ISO 8601, from EXIF if available
    owner: {
      type: "object", required: false, default: null,
      shape: {
    ...
    created_at:   { type: "string",  required: true,  default: () => new Date().toISOString() }, // ISO 8601 UTC
    updated_at:   { type: "string",  required: true,  default: () => new Date().toISOString() },
    resolved_at:  { type: "string",  required: false, default: null },
    history:      { type: "array",   required: false, default: [] }, // [{ at, by, action, from, to }]
    metadata:     { type: "object",  required: false, default: null } // captureMetadata() blob
  };
    ...
    for (const key in FIELDS) {
      ...
      if (spec.shape && actual === "object") {
        for (const sk in spec.shape) {
          ...
          const sv = v[sk];
          if (ss.required && (sv === undefined || sv === null)) {
            errors.push(key + "." + sk + " is required");
            continue;
          }
          if (sv !== undefined && sv !== null && typeOf(sv) !== ss.type) {
            errors.push(key + "." + sk + " expected " + ss.type);
          }
        }
      }
    }
  ...
  // Coerces a partial / legacy / external payload into a canonical Report.
  // Maps legacy field names (camelCase) onto the canonical (snake_case)
  // shape so historical Firestore docs keep working.
  function normalize(input) {
    ...
    for (const a in aliases) {
      if (merged[a] !== undefined && merged[aliases[a]] === undefined) {
        merged[aliases[a]] = merged[a];
      }
    }
    ...
    for (const key in FIELDS) {
      ...
      // ISO coercion for date-ish fields
      ...
      if (spec.shape && typeof v === "object" && !Array.isArray(v)) {
        ...
        for (const sk in spec.shape) {
          ...
          if (nv === undefined || nv === null) {
            nested[sk] = ss.default !== undefined ? defaultFor(ss) : null;
          } else {
            nested[sk] = nv;
          }
        }
    ...
    // Always re-stamp updated_at if status changed (defensive)
    if (r.status === Status.RESOLVED && !r.resolved_at) {
      r.resolved_at = r.updated_at || new Date().toISOString();
    }
    return r;
  }

  // ---------- toOpen311(report) ----------
  // Map a canonical Report to a GeoReport v2 (Open311) ServiceRequest object.
  // Spec: https://wiki.open311.org/GeoReport_v2/
  function toOpen311(r) {
    const statusMap = {
      open: "open", in_progress: "open", resolved: "closed", closed: "closed"
    };
    const out = {
  ...
  // ---------- toFlatRow(report) ----------
  // Flat row for CSV / Excel / SQL ingestion.
  ...
  // ---------- Status transitions ----------
  // Returns true if next is reachable from current (defensive for the kanban).
  const TRANSITIONS = {
